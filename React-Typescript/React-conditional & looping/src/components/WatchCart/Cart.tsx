...
const Cart: React.FC = () => {
  ...
  // const total = watchs.reduce((acc, item) => {
  //   return acc + item.qty * item.price;
  // }, 0);

  let grandTotal = 0;
  const total = watchs.map(
    (item) => (grandTotal = grandTotal + item.qty * item.price)
  );
  ...
  return (
    <div>
      <div className="container mt-3">
        ...
        {watchs.length > 0 && watchs ? displayTable() : displayNoProducts()}
      </div>
    </div>
  );

  function displayNoProducts(): React.ReactNode {
    return (
      <div className="row">
        <div className="col-sm-12">
  ...
  function displayTable(): React.ReactNode {
    return (
